...
# Computational study on the rice dataset
...
get_gamma <- function(str) {
  round(as.numeric(strsplit(str, ",")[[1]][1]), 4)
}
get_w <- function(str) {
  params <- strsplit(str, ",")[[1]]
  w <- c()
  for (i in 2:length(params)) {
    wi <- round(as.numeric(params[i]), 4)
    w <- unlist(c(w, wi))
  }
  w
}
data$gamma <- unlist(lapply(data$params, get_gamma))
data$w <- lapply(data$params, get_w)
