...
We will now test the primal and dual formulations on a dataset generated using R, in order to ensure that, as expected, primal and dual formulations of the SVM give the same results.
...
Using Gauss Kernel with sigma square 0.1 we obtain the following classificator. This kernel transforms the data into a new Hilbert space that allows the SVM to classify non-linear data in the original space. We know beforehand that this dataset will be much better classified using this kernel as the data is not linearly separable.
...
We can see that median accuracy increases with $\nu$, topping out at $\nu = 1$.
...
boxplot(acc ~ model, data =data, xlab = "Model", ylab="Accuracy", col = 10:15, outline=FALSE)
```
