...
classify <- function(data_file_name, w, gamma, num_features) {
  data <- read.table(data_file_name)
  data["predict"] <- apply(df, 2, function(x) t(w) %*% as.numeric(x[1:num_features]) + gamma > 0)
  data
}
```
...
for (nu in c(0.1, 0.5, 1, 5)) {
  ...
  plot(classified$V1, classified$V2, col=ifelse(classified$predict, "red", "black"))
  ncor <- svm_acc[1]
  nts <- svm_acc[2]
...
We can see that, on this dataset, accuracy is not very high across values of $\nu$ if a linear classifier is used. A comprehensive and quantitative comparison will be performed in the following section.
