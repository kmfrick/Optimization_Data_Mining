...
boxplot(cpusec ~ model, data = cputime_data, xlab = "Model", ylab="CPU time (s)", col = 10:15, outline=FALSE)
```
...
boxplot(cpusec ~ num_features, data = dual_cputime, col =10:15, ylab="CPU time(s)", xlab="Number of data points")
```
...
print(select(primal_cputime, dataset,num_features,cpusec))
```
