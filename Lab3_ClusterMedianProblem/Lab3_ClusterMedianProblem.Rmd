...
plot(seeds.pca$scores[,1:2], col=seeds[,8])
```
...
```{r}
ggnet2(seeds.dist.mst, node.size = 3, node.color = seeds.mst.colors, directed=FALSE, mode = seeds.pca$scores)
```
...
```{r}
seeds.confusion.matrix.mst <- get.confusion.matrix.mst(seeds[,8], clust.mst, seeds.mst.median.nodes)
...
ggnet2(blobs.dist.mst, node.size = 3, node.color = blobs.mst.colors, directed=FALSE, mode = as.matrix(blobs))
```
