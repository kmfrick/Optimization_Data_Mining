...
par(mfrow=c(2,2))
invisible(apply(seeds[c(2,10,183,50),], 1, function(x) plot.grain(0,0,x[5],x[4],scale=0.3,add=FALSE)))
```
...
We can see that this value is slightly more than double what is obtained by solving the optimization problem with AMPL, confirming a low-quality clustering.
...
# Principal components of wheat varieties
...
We therefore run MST-based clustering generating the distance matrix using only the first two principal components calculated earlier.

```{r}
seeds.pca.dist <- as.matrix(dist(seeds.pca$scores[,1:2]))
seeds.pca.edge.list <- gen.edge.list(seeds.pca.dist)
m <- nrow(seeds.pca.dist)
seeds.pca.mst.edge.list <- kruskal(seeds.pca.edge.list, m)
k <- 3
n.edges <- nrow(seeds.pca.mst.edge.list)
seeds.pca.mst.edge.list <- seeds.pca.mst.edge.list[1:(n.edges - (k - 1)),]
seeds.pca.dist.mst <- gen.dist.mst(seeds.pca.mst.edge.list, m)
...
```{r}
seeds.pca.child.list.mst <- gen.child.list.mst(seeds.pca.mst.edge.list, m)
# Calculate the median nodes to set the correct color for each cluster median
seeds.pca.clust.mst <- mst.cluster(seeds.pca.child.list.mst, m)
seeds.pca.mst.median.nodes <- get.mst.median.nodes(seeds.pca.clust.mst,
                                                   seeds.pca.dist, k)
seeds.pca.clust.mst <- as.factor(seeds.pca.clust.mst)
levels(seeds.pca.clust.mst) <- seeds[seeds.pca.mst.median.nodes,8]
seeds.pca.clust.mst <- as.numeric(as.character(seeds.pca.clust.mst))
seeds.pca.mst.colors <- gen.palette(seeds.pca.clust.mst, nrow(seeds.pca$scores))
ggnet2(seeds.pca.dist.mst, node.size = 3, node.color = seeds.pca.mst.colors,
       directed=FALSE, mode = as.matrix(seeds.pca$scores)) +
  labs(title = "Trees/clusters generated by MST") +
  theme(plot.title = element_text(hjust = 0.5))
plot(c(), xlab="Component 1", ylab="Component 2", opacity=.3,
     xlim = range(X), ylim = range(Y), main = "'Gold standard' clusters")
...
```
We can see that, even in this case, the quality of clustering is very poor.
We now print the value of the objective function given by the MST heuristic.
...
Again, the value is slightly more than double what was obtained before using AMPL, indicating that the dimensionality is not a factor in helping separate clusters.
...
In this section we will compare the results obtained by solving the integer programming formulation of the cluster median problem and those obtained with an MST-based heuristic.
...
On the other hand, concerning an MST-based heuristic, our experiments show two cases, one where said heuristic performs suboptimally, because nearly all points are assigned to the same cluster and another one, that shows that if clusters are clearly separated and don't overlap, the result is the same as the optimal solution.
We have shown that accuracy is not improved if the dimensionality of the dataset is reduced, and that in a dataset with poor separability between clusters, the value of the objective function obtained with the heuristic is more than double the one given by the optimal solution.

## Theoretical and experimental evaluation of complexity

Finding the exact solution for the cluster median problem has been proven to be an NP-hard problem by @megiddo1984complexity and no polynomial time algorithm is known yet.

The heuristic we studied uses Kruskal's algorithm which allows for computing the MST in $O(E\log{V})$.

The exact solution on medium-size instances is too hard for classical computers, so approximate algorithms such as the one presented in this document become a necessity and are among the best known methods for solving them.

To empirically confirm these theoretical predictions, we perform 100 runs of each dataset and measure the time taken by each run.
...
We can see that for a dataset with clusters that are overlapping and not well-isolated or linearly separable, like the wheat seeds dataset we used, solving the problem using the minimum spanning tree as heuristic provides for a very poor quality clustering, with a value of the objective function that is more than double the optimal one and accuracy with respect to the gold standard that is barely better than random guessing.

However, if the dataset shows noticeable linear separability between clusters, our heuristic becomes much more effective, being able to obtain the optimal solution in a small fraction of the time required by the optimization problem to be solved.

In any case, the exact solution becomes unfeasible very quickly in large datasets, so if the heuristic detailed here does not provide satisfactory results, it is necessary to search for a more suitable heuristic or settle with sub-optimal approximations.
